...
class DataFiltering(object):
    def __init__(self, repository):
        self.self.repository = repository

    def pre_processing(self, force=False):
        """
        ...
        """
        try:
            logging.debug(f"Start the filtering in '{self.repository}'")

            if os.path.isfile(f"{self.repository}/data-filtered.csv") and not force:
                logging.debug(f"File already exists! Skipping...")
                return

            df_repo = pd.read_json(f"{self.repository}/repo-data-travis.json")
            ...
            df_buildlog = pd.read_json(f"{self.repository}/buildlog-data-travis.json")
            ...
            df_new.to_csv(f'{self.repository}/data-filtered.csv', sep=";", index=False)
            logging.debug(f"Finish filtering, saving Filtered CSV for '{self.repository}'")
        except Exception as e:
            exc_type, exc_obj, exc_tb = sys.exc_info()
            logging.error(f'Failed to filtering CSV in line {exc_tb.tb_lineno}: {str(e)}')

    def feature_engineering(self, force=False):
        """
        This function get the Filtered Data and parse it to a file (features engineered CSV)
        that it will be used in the analysis
        """
        try:
            logging.debug(f"Start the feature engineering procedure in '{self.repository}'")

            if os.path.isfile(f"{self.repository}/features-engineered.csv") and not force:
                logging.debug(f"File already exists! Skipping...")
                return

            df = pd.read_csv(
                '{}/data-filtered.csv'.format(self.repository), sep=";", parse_dates=True)
            ...
            df['travis_started_at'] = df['travis_started_at'].apply(
                lambda date: datetime.datetime.strptime(date.split('+')[0] if '+' in date else date,
                                                        '%Y-%m-%d %H:%M:%S'))
            ...
            logging.debug(f"Store results in '{self.repository}/features-engineered.csv'")
            tcdf.to_csv(f"{self.repository}/features-engineered.csv", sep=';', na_rep='[]',
                        columns=tc_fieldnames,
                        header=True, index=False,
                        quoting=csv.QUOTE_NONE)

            logging.debug(f"Finish the feature engineering procedure in '{self.repository}'")
        except Exception as e:
            exc_type, exc_obj, exc_tb = sys.exc_info()
            logging.error(f'Failed to feature engineering in line {exc_tb.tb_lineno}: {str(e)}')

    def add_last_results(self, force=False):
        try:
            logging.debug(f"Start the feature engineering procedure in '{self.repository} to add last results'")

            if os.path.isfile(f"{self.repository}/features-engineered_lastresults.csv") and not force:
                logging.debug(f"File already exists! Skipping...")
                return

            df = pd.read_csv(
                '{}/features-engineered.csv'.format(self.repository), sep=";", parse_dates=True)
            ...
            logging.debug(f"Store results in '{self.repository}/features-engineered_lastresults.csv'")
            df.to_csv(f"{self.repository}/features-engineered_lastresults.csv", sep=';', na_rep='[]',
                      header=True, index=False,
                      quoting=csv.QUOTE_NONE)

            logging.debug(f"Finish the feature engineering procedure in '{self.repository}' to add last results")

        except Exception as e:
            exc_type, exc_obj, exc_tb = sys.exc_info()
            logging.error(f'Failed to add last results in line {exc_tb.tb_lineno}: {str(e)}')

    def parse_industrial_dataset(self, force=False):
        try:
            logging.debug(f"Start the feature engineering procedure in '{self.repository}', an industrial dataset")
            ...
            logging.debug(f"Store results in '{self.repository}/features-engineered.csv'")
            df.to_csv(f"{self.repository}/features-engineered.csv", sep=';', na_rep='[]',
                      header=True, index=False,
                      quoting=csv.QUOTE_NONE)

            logging.debug(f"Finish the feature engineering procedure in '{self.repository}', an industrial dataset")
